const Header = () => {
  return (
    <header className="flex flex-row items-center justify-center align-center w-full py-5 sm:py-10 font-[playfair]">
      <h1 className="flex flex-col align-center content-center justify-center text-center text-4xl font-bold">
        <span className="text-xl">The</span>
        <span className="text-blue-700 dark:text-blue-300 text-5xl">
          Butterfly
        </span>
        <span className="">Insights</span>
      </h1>
    </header>
  );
};

export default Header;
